...
export function liveTelegram(env = process.env) {
  ...
  // Operator/ops alerts go to a SEPARATE chat (e.g. the operator's DM), NEVER the public card group.
  // If TELEGRAM_ALERT_CHAT_ID is not set, alerts are console-only (the group stays clean).
  const alertChatId = env.TELEGRAM_ALERT_CHAT_ID;
  ...
  return {
    async alert(text) {
      if (!token || !alertChatId) { console.warn(`[telegram alert] ${text}`); return { skipped: true }; }
