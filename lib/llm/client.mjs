...
export function liveLLM(env = process.env) {
  ...
  return {
    ...
    async tagItems(items) {
      ...
      const prompt =
        `For each numbered headline, extract the key entities (CANONICAL full team and person names, ` +
        `e.g. "Wolverhampton Wanderers","Rob Edwards") and one event_type from ` +
        `[transfer, match_result, injury, signing, sacking, preview, other]. Use the SAME canonical ` +
        `names across headlines about the same event so they can be matched.\n\n${numbered}\n\n` +
        `Return a JSON array aligned 1..${items.length}: [{"entities":["..."],"eventType":"..."}].`;
      const out = await callJSON({ model: extractModel, system, prompt, temperature: 0, maxTokens: 6000 });
      const arr = Array.isArray(out) ? out : (out.items ?? out.tags ?? []);
      return items.map((_, i) => ({ entities: arr[i]?.entities ?? [], eventType: arr[i]?.eventType ?? 'other' }));
    },
...
export function fixtureLLM(fixtures) {
  return {
    ...
    async tagItems(items) {
      return items.map((it) => ({ entities: it.entities ?? [], eventType: it.eventType ?? 'other' }));
    },
    async cardSummary({ article, lang }) {
      const title = lang === 'ko' ? article.titleKo : article.titleEn;
      const body = (lang === 'ko' ? article.bodyKo : article.bodyEn) || '';
