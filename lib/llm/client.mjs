...
export function liveLLM(env = process.env) {
  ...
  return {
    ...
    // Semantic same-event dedup: is `title` the SAME real-world event as any published candidate?
    async isDuplicateEvent({ title, candidates = [] }) {
      ...
      const system = 'You detect duplicate sports news events. Output strictly valid JSON only.';
      const prompt =
        `New article headline:\n"${title}"\n\nAlready-published headlines:\n${list}\n\n` +
        `Does the NEW headline describe the SAME real-world sports event (same match/result, transfer, ` +
        `game) as any already-published one? Different games/teams/dates = NOT the same. ` +
        `Return ONLY JSON: {"duplicate": true|false, "index": <matching number or 0>}`;
      try {
        const out = await callJSON({ model: extractModel, system, prompt, temperature: 0, maxTokens: 200 });
...
export function fixtureLLM(fixtures) {
  return {
    ...
    // Dry-run: fixtures are distinct events; cross-run idempotency is covered by fingerprint/URL dedup.
    async isDuplicateEvent() { return { duplicate: false }; },
  };
}
