...
export async function assessStoryArt(artBuf, beats, env = process.env) {
  ...
  const instruction =
    ...
    `Rate how well it works as a coherent 4-beat story. Penalize: empty/blank panels, a panel that is ` +
    `just a crowd/background with no action, duplicated panels, garbled anatomy, or panels that do not ` +
    `match the beats. Reply ONLY JSON: {"score":<0..1>,"reason":"<short>"} (score >= 0.6 = acceptable).`;
  try {
