...
// Transient Gemini failures (429 rate-limit, 5xx, network) are RETRIED with backoff — the biggest
// real-world failure is the free-tier RPM limit tripping after we've tagged a day's worth of news,
// which used to silently yield a placeholder card. `ok` distinguishes real art from the fallback so
// callers can refuse to ship an empty card.
const IMG_RETRIES = Number(process.env.GEMINI_IMAGE_RETRIES ?? 3);
const isTransient = (status) => status === 429 || status === 408 || status >= 500;
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

export async function generateComicImage(prompt, env = process.env, { aspectRatio = COMIC_ASPECT } = {}) {
  const key = env.GEMINI_API_KEY;
  if (!key) return { buffer: await placeholderPanel(), source: 'placeholder(no-key)', ok: false };
  ...
  for (let attempt = 0; attempt <= IMG_RETRIES; attempt++) {
    try {
      ...
      if (!res.ok) {
        ...
        if (isTransient(res.status) && attempt < IMG_RETRIES) {
      ...
      return { buffer, source: GEMINI_IMAGE_MODEL, ok: true };
    } catch (err) {
      lastErr = err.message;
      if (attempt < IMG_RETRIES && /fetch failed|network|ECONN|ETIMEDOUT/i.test(err.message)) {
  ...
  return { buffer: await placeholderPanel(), source: `placeholder(${lastErr})`, ok: false };
}

// Like generateComicImage, but conditions generation on a REFERENCE PHOTO (image-to-image) so the
// manga protagonist resembles a real athlete. Same retry/fail-soft contract. The photo is the first
// part; the text prompt (which must instruct "resemble the person in the reference photo") is second.
export async function generateComicImageFromPhoto(prompt, photoBuf, env = process.env, { aspectRatio = COMIC_ASPECT } = {}) {
  const key = env.GEMINI_API_KEY;
  if (!key || !photoBuf?.length) return { buffer: await placeholderPanel(), source: 'placeholder(no-photo)', ok: false };
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_IMAGE_MODEL}:generateContent?key=${key}`;
  let lastErr = 'unknown';
  for (let attempt = 0; attempt <= IMG_RETRIES; attempt++) {
    try {
      ...
      if (!res.ok) {
        const body = (await res.text()).slice(0, 200);
        if (isTransient(res.status) && attempt < IMG_RETRIES) { lastErr = `Gemini ${res.status}`; await sleep(1500 * 2 ** attempt); continue; }
        throw new Error(`Gemini ${res.status}: ${body}`);
      }
      const json = await res.json();
      const img = (json?.candidates?.[0]?.content?.parts || []).find((p) => p.inlineData?.data);
      if (!img) throw new Error('no image part in response');
      const buffer = await sharp(Buffer.from(img.inlineData.data, 'base64')).png().toBuffer();
      return { buffer, source: `${GEMINI_IMAGE_MODEL}(photo)`, ok: true };
    } catch (err) {
      lastErr = err.message;
      if (attempt < IMG_RETRIES && /fetch failed|network|ECONN|ETIMEDOUT/i.test(err.message)) { await sleep(1500 * 2 ** attempt); continue; }
      break;
    }
  }
  return { buffer: await placeholderPanel(), source: `placeholder(${lastErr})`, ok: false };
...
// Quality guard: does the generated 4-panel art actually tell the intended story? A vision pass
// (gemini flash is multimodal) scores 0..1 and flags broken/empty/duplicated panels. Fail-soft:
// returns score 1 (accept) on any error so a flaky check never blocks a good card.
export async function assessStoryArt(artBuf, beats, env = process.env) {
  const key = env.GEMINI_API_KEY;
  if (!key || !artBuf?.length) return { score: 1, reason: 'no-check' };
  const want = beats.map((b, i) => `${i + 1}. ${b.scene}`).join('\n');
  const instruction =
    `This image is a 4-panel manga (2x2 grid) meant to tell this sports story in order:\n${want}\n\n` +
    `Rate how well it works as a coherent 4-beat story, 0..1 (>= 0.6 = acceptable). HARD RULE: if ANY of ` +
    `the 4 panels is empty/blank, is only speed lines, is only background or crowd with no clearly drawn ` +
    `character, or has a main figure with its head/body cropped off at the panel edge, the score MUST be ` +
    `<= 0.3. Also penalize duplicated panels, garbled anatomy, or panels that don't match the beats. ` +
    `Reply ONLY JSON: {"score":<0..1>,"reason":"<short>"}.`;
  try {
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_TEXT_MODEL}:generateContent?key=${key}`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        contents: [{ parts: [
          { inline_data: { mime_type: 'image/png', data: artBuf.toString('base64') } },
          { text: instruction },
        ] }],
    ...
    return { score: Number.isFinite(score) ? score : 1, reason: out.reason || '' };
  } catch { return { score: 1, reason: 'check-failed' }; }
}

// Per-panel quality check: is this SINGLE manga panel a usable frame (a clear character doing the
// action, nothing cropped, not an empty/burst frame)? Score 0..1, >= 0.6 acceptable. Fail-soft → 1.
export async function assessPanel(panelBuf, scene, env = process.env) {
  const key = env.GEMINI_API_KEY;
  if (!key || !panelBuf?.length) return { score: 1 };
  const instruction =
    `This is one manga panel meant to show: "${scene}". Score 0..1 how usable it is (>= 0.6 = ok). ` +
    `HARD RULES (any of these → score <= 0.2): the panel is empty/blank; it is only speed lines; it is a ` +
    `bright explosion / light burst / impact flash / glowing blast with no clearly drawn person; it is just ` +
    `background or crowd with no clear character. ALSO score <= 0.3 if: the main figure's head or body is ` +
    `cropped at an edge, OR a face is left blank/featureless/white. Reply ONLY JSON: {"score":<0..1>}.`;
  try {
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_TEXT_MODEL}:generateContent?key=${key}`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ contents: [{ parts: [
        { inline_data: { mime_type: 'image/png', data: panelBuf.toString('base64') } },
        { text: instruction },
      ] }], generationConfig: { temperature: 0 } }),
    });
    if (!res.ok) throw new Error(`Gemini ${res.status}`);
    ...
    const txt = (json?.candidates?.[0]?.content?.parts?.map((p) => p.text).join('') || '').trim();
    const m = txt.match(/\{[\s\S]*\}/);
    const score = Number((m ? JSON.parse(m[0]) : {}).score);
    return { score: Number.isFinite(score) ? score : 1 };
  } catch { return { score: 1 }; }
}

// Strip any text the image model drew anyway (scoreboards, signage, SFX words, speech bubbles) —
// image models garble Korean, so we blank those surfaces via a Gemini image-EDIT pass. Returns the
// cleaned PNG, or the original on failure (fail-soft).
export async function removeImageText(imgBuf, env = process.env) {
  const key = env.GEMINI_API_KEY;
  if (!key) return imgBuf;
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_IMAGE_MODEL}:generateContent?key=${key}`;
  const instruction = 'Edit this black-and-white manga image: REMOVE all text, letters, numbers, words, '
    + 'scoreboards, banners, signs, speech bubbles, and any written characters (Korean or otherwise). '
    + 'Leave those surfaces blank and clean. Keep the artwork, characters, panels, screentone, and '
    + 'composition EXACTLY the same otherwise.';
  try {
    const res = await fetch(url, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ contents: [{ parts: [{ inline_data: { mime_type: 'image/png', data: imgBuf.toString('base64') } }, { text: instruction }] }] }),
    });
    if (!res.ok) throw new Error(`Gemini ${res.status}`);
    const json = await res.json();
    const img = (json?.candidates?.[0]?.content?.parts ?? []).find((p) => p.inlineData?.data);
    if (!img) throw new Error('no image part');
    return await sharp(Buffer.from(img.inlineData.data, 'base64')).png().toBuffer();
  } catch { return imgBuf; }
}
