...
export async function deriveTwoBeats({ sportKey, headline, summary = '', eventType = 'other' }, env = process.env) {
  ...
  const instruction =
    ...
    `match the guidance and ONLY depict what actually happened (never invent a result). Use generic ` +
    `anonymous athletes — NO real names, NO real-person faces, NO team logos, NO text/numbers. ` +
...
export function mangaPageTwoCutPrompt({ beat1, beat2 }) {
  return `An authentic black-and-white Japanese shonen MANGA PAGE with EXACTLY TWO panels of similar ` +
    ...
    `foreshortening, speed lines and focus lines for impact. Generic anonymous athletes — NO real ` +
    `faces, NO team logos. Render NO readable text, NO letters, NO numbers, NO speech bubbles ` +
    `(leave the art clean). It must clearly look like two panels from a manga book.`;
}
