...
export async function deriveFourBeats({ sportKey, headline, summary = '', eventType = 'other' }, env = process.env) {
  ...
  const instruction =
    ...
    `EVERY panel MUST show at least one person performing a clear, visible action — NEVER an empty field, ` +
    `an abstract "tension"/mood shot, or a plain background; if a beat is quiet (e.g. a player waiting), ` +
    `still show the person doing something concrete. Describe framing so the main figures are fully in view. ` +
...
// When `likeness` is set, a reference PHOTO is supplied alongside this prompt (image-to-image): the
// recurring protagonist across all 4 panels should RESEMBLE that person; otherwise athletes are generic.
export function fourCutPrompt(beats, { likeness = false } = {}) {
  const faces = likeness
    ? `The recurring MAIN athlete in the panels should RESEMBLE the person in the provided reference photo `
      + `(same face shape, eyes, hairstyle, build) so they are recognizable; secondary figures are generic. `
      + `NO team logos.`
    : `Generic anonymous athletes, NO real-person faces, NO team logos.`;
  return `A black-and-white Japanese shonen MANGA PAGE, EXACTLY 4 equal panels in a 2x2 grid — each `
    ...
    + ` ${COMIC_STYLES.shonen} Heavy screentone, dramatic angles, foreshortening, speed lines. ${faces} `
    + `EVERY one of the 4 panels MUST contain clearly drawn characters performing visible action — NO empty `
    + `panel, NO panel that is only speed lines or blank background. In each panel keep the main figures FULLY `
    + `inside the panel with margin from all edges — do NOT crop heads, faces, or bodies at the panel borders. `
    + `Render absolutely NO text anywhere — NO speech bubbles, NO captions, NO scoreboards/banners/signage `
    + `with letters or numbers (blank surfaces).`;
}

// Prompt for a SINGLE panel (we generate the 4 panels separately, then composite — far more reliable
// than asking one image for a 2x2 grid, which drops/empties/crops panels). When `likeness` is set a
// reference photo is supplied alongside (image-to-image) so this panel's athlete resembles that person.
export function panelPrompt(scene, { likeness = false } = {}) {
  const faces = likeness
    ? `The main athlete RESEMBLES the person in the provided reference photo (same face shape, eyes, `
      + `hairstyle, build) so they are recognizable; other figures generic. NO team logos.`
    : `Generic anonymous athletes, NO real-person faces, NO team logos.`;
  return `A SINGLE black-and-white Japanese shonen MANGA PANEL showing: `
    + `${scene}. ${COMIC_STYLES.shonen} Heavy screentone, dramatic angle, foreshortening, speed lines. `
    + `${faces} A clearly drawn PERSON must be the focus of the frame performing visible action. Do NOT `
    + `replace the character with an abstract explosion, light burst, impact flash, glowing blast, or a plain `
    + `background — no empty frames, no "only speed lines" frames. Every face must be fully drawn with visible `
    + `facial features (eyes, nose, mouth) — never leave a face blank, white, glowing, or washed out by light. `
    + `Keep the main figure(s) FULLY `
    + `inside the frame with margin from all edges — do NOT crop heads or bodies at the edges. The artwork `
    + `fills the ENTIRE square edge-to-edge with NO white border or margin around it. Render absolutely NO `
    + `text — NO speech bubbles, NO captions, NO scoreboards/banners/signage with letters or numbers.`;
}

// Composite 4 square panel buffers into one 2x2 square (uniform white gutters), matching gutterSvg's
// quadrant geometry so renderFourCutCard's border overlay lines up. Guarantees 4 filled panels.
export async function composeFourPanels(buffers, S = 1024) {
  const c = S / 2, G = 22, hg = G / 2, cell = c - hg;
  const pos = [[0, 0], [c + hg, 0], [0, c + hg], [c + hg, c + hg]];
  const cells = await Promise.all(buffers.map((b) =>
    sharp(b).resize({ width: cell, height: cell, fit: 'cover', position: 'attention' }).png().toBuffer()));
  const bg = `<svg width="${S}" height="${S}" xmlns="http://www.w3.org/2000/svg"><rect width="${S}" height="${S}" fill="#fff"/></svg>`;
  return sharp(Buffer.from(bg))
    .composite(cells.map((input, i) => ({ input, left: pos[i][0], top: pos[i][1] })))
    .png().toBuffer();
}
