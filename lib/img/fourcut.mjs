...
// Uniform 2x2 gutters + panel borders imposed by US (Gemini's own gutters are uneven). White gutters
// of equal thickness horizontally and vertically, with a thin ink border around each panel.
...
export async function renderFourCutCard({ sportKey, date = '', headline, mangaBuffer, dialogues, accent }) {
  ...
  // square the art, impose UNIFORM gutters + panel borders, THEN overlay the Korean bubbles on top.
  ...
  const manga = await overlayBubbles(sq1, dialogues);
